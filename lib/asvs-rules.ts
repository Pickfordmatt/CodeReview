...
export const asvsRules: SecurityRule[] = [
  // V2: Authentication
  {
    ...
    severity: 'critical',
    pattern: /(password|passwd|pwd|secret|api[_-]?key|apikey|token)\s*=\s*["'][^"']{3,}["']/gi,
    level: 1,
  },
  {
    id: 'V2.1.2',
  ...
  },
  {
    ...
    description: 'Cryptographically secure random number generators should be used',
    severity: 'medium',
  ...
  },
  {
    ...
    description: 'Stack traces should not be exposed to users',
    severity: 'medium',
    pattern: /(printStackTrace|print_r|var_dump|console\.trace)\s*\(/gi,
    level: 2,
  },
  ...
  // V9: Communications
  {
    ...
    description: 'All connections should use HTTPS',
    severity: 'high',
  ...
  // V13: API and Web Service
  {
    ...
    severity: 'medium',
    pattern: /app\.(get|post|put|delete)\s*\(.*async.*\)(?!.*rateLimit)/gi,
    level: 2,
  },
  {
    id: 'V13.2.1',
  ...
  // V14: Configuration
  {
    ...
    severity: 'high',
    pattern: /(DEBUG|ENVIRONMENT).*=.*(true|development|dev)(?!.*production)/gi,
    level: 1,
  },
];
